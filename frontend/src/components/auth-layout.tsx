...
export function AuthLayout({children, title, subtitle, sideContent}: AuthLayoutProps) {
    return (
        <div className="min-h-screen w-full lg:grid lg:grid-cols-2">

            {/* PANEL IZQUIERDO (Visual - Branding) */}
            <div
                className="hidden lg:flex flex-col items-center justify-center relative bg-zinc-900 text-white overflow-hidden border-r border-zinc-800">

                {/* Fondo decorativo tech */}
                <div
                    className="absolute inset-0 bg-[radial-gradient(circle_at_center,var(--tw-gradient-stops))] from-zinc-800 via-zinc-900 to-zinc-950 opacity-50"/>
                ...
                <div className="relative z-10 flex flex-col items-center text-center space-y-8 max-w-lg px-8">
                    ...
                    <div className="relative group">
                        <div
                            className="absolute -inset-1 bg-linear-to-r from-blue-600 to-cyan-600 rounded-full blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200"/>
                    ...
                    <div className="space-y-4">
                        <h2 className="text-3xl font-bold tracking-tight">{sideContent.title}</h2>
            ...
            <div className="flex items-center justify-center p-8 bg-background">
                <div className="mx-auto w-full max-w-100 space-y-6">
                    ...
                    <p className="px-8 text-center text-sm text-muted-foreground lg:hidden">
                        {sideContent.ctaText} <Link href={sideContent.ctaLink}
                                                    className="underline underline-offset-4 hover:text-primary">{sideContent.ctaLabel}</Link>
                    </p>
                </div>
            </div>
        </div>
